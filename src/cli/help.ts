...
export function formatHelpMessage(useAnsi: boolean): string {
	...
	return `${bold("flatr")} ${dim("—")} Flatten your codebase into a single file for LLMs.

${bold("Usage:")} flatr ${cyan("[directory]")} ${dim("[...flags]")}
...
${bold("Options:")}
  ...
  ${cyan("    --no-sponsor-message")}  Hide support message ${dim("(also: FLATR_NO_SPONSOR=1)")}
...
${bold("Examples:")}
  ${dim("$")} flatr . ${cyan("-o")} output.md
  ${dim("$")} flatr src ${cyan("-e")} ${green('"*.test.ts"')} ${cyan("-e")} ${green('"fixtures/"')}
  ${dim("$")} flatr . ${cyan("--no-contents --format")} json
`;
}
