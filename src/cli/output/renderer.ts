...
export class OutputRenderer {
	...
	#renderNormal(data: SuccessData): void {
		...
		if (parts.length > 0) {
			...
			console.info(
				`${symbol} ${applyColor(text, colors.dim, this.#useColors)}`,
			);
		}
		...
		if (warnings.length > 0) {
			...
			if (warningsText) console.info(warningsText);
		}

		if (result.errors > 0) {
			const errorsText = renderErrors([], this.#useColors, false);
			if (errorsText) console.error(errorsText);
		}
	}
	...
	#renderVerbose(data: SuccessData): void {
		...
		const filesLabel = applyColor(
			"Files".padEnd(maxLabelLength),
			colors.dim,
			this.#useColors,
		);
		const filesProcessed = applyColor(
		...
		const filesErrors =
			result.errors > 0
				? `, ${applyColor(result.errors, colors.error, this.#useColors)} errors`
				: "";

		console.info(
			`${filesLabel}  ${filesProcessed} processed${filesSkipped}${filesBinary}${filesErrors}`,
		);

		const tokensLabel = applyColor(
			"Tokens".padEnd(maxLabelLength),
			colors.dim,
			this.#useColors,
		);
		...
		const timeLabel = applyColor(
			"Time".padEnd(maxLabelLength),
			colors.dim,
			this.#useColors,
		);
		const timeValue = applyColor(
