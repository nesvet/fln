...
import type { FlatrConfig } from "$config";
...
async function writeMarkdown(result: ScanResult, config: FlatrConfig): Promise<void> {
	...
	try {
		await writer.writeLine(`<!-- 🥞 flatr ${VERSION} -->`);
...
async function writeJson(result: ScanResult, config: FlatrConfig): Promise<void> {
	const writer = await createOutputWriter(config.outputFile, config.maximumTotalSizeBytes);
	const outputRoot = filterSkippedNodes(result.root);
...
export async function writeOutput(result: ScanResult, config: FlatrConfig): Promise<void> {
