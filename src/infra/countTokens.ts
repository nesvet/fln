...
(function initializeAsciiMap() {
	...
	for (let i = 0x30; i <= 0x39; i++) ASCII_MAP[i] = CharCategory.Digit;

	for (let i = 0x41; i <= 0x5a; i++) ASCII_MAP[i] = CharCategory.Latin;

	for (let i = 0x61; i <= 0x7a; i++) ASCII_MAP[i] = CharCategory.Latin;
...
export function countTokens(text: string): number {
	...
	while (index < length) {
		...
		if (code < 128) category = ASCII_MAP[code] as CharCategory;
		...
		switch (category) {
			case CharCategory.Latin: {
				...
				while (wordEnd < length) {
					nextCode = text.charCodeAt(wordEnd);
					if (
						!(
				...
				if (COMMON_WORDS.has(word) || wordLength <= 3) tokenCount++;
				else
					tokenCount += Math.max(
						1,
						Math.round(wordLength / LATIN_CHARS_PER_TOKEN),
					);

				if (hasContraction) tokenCount++;
			...
			case CharCategory.Digit: {
				...
				if (
					index > 0 &&
					text.charCodeAt(index - 1) === 0x78 &&
					index > 1 &&
					text.charCodeAt(index - 2) === 0x30
				) {
					hasHexPrefix = true;
					while (digitEnd < length) {
						...
						if (
							!(
								...
								(nextCode >= 0x41 && nextCode <= 0x46) ||
								(nextCode >= 0x61 && nextCode <= 0x66)
							)
						)
							break;
						digitEnd++;
					...
					while (digitEnd < length) {
						const nextCode = text.charCodeAt(digitEnd);
						if (nextCode < 0x30 || nextCode > 0x39) break;
						digitEnd++;
					}

				const digitCount = digitEnd - index;

				if (hasHexPrefix || digitCount === 4) tokenCount++;
				else if (digitCount <= 2) tokenCount++;
				else tokenCount += Math.ceil(digitCount / 3);
			...
			case CharCategory.Cyrillic: {
				...
				while (
					index < length &&
					text.charCodeAt(index) >= CYR_START &&
					text.charCodeAt(index) <= CYR_END
				);

				const charCount = index - start;
				tokenCount += Math.max(
					1,
					Math.round(charCount / CYRILLIC_CHARS_PER_TOKEN),
				);
				break;
			}
			...
			case CharCategory.Arabic: {
				...
				while (
					index < length &&
					text.charCodeAt(index) >= ARABIC_START &&
					text.charCodeAt(index) <= ARABIC_END
				);

				const charCount = index - start;
				tokenCount += Math.max(
					1,
					Math.round(charCount / ARABIC_CHARS_PER_TOKEN),
				);
				break;
			}
