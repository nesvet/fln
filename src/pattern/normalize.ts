...
function normalizePatternBody(
	...
	const trimmedTrailingSlash = withoutLeadingDot.endsWith("/")
	...
	const pathPrefix =
		globIndex >= 0
			? trimmedTrailingSlash.slice(0, globIndex)
			: trimmedTrailingSlash;
	const globSuffix =
		globIndex >= 0 ? trimmedTrailingSlash.slice(globIndex) : "";
...
export function normalizeExcludePattern(
	pattern: string,
	base: string,
): string | null {
...
export function normalizeIncludePattern(
	pattern: string,
	base: string,
): string | null {
