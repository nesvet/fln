...
import { FlnError } from "../src/api/flnError.js";
import {
	getProjectMetadata,
	loadConfigFile,
	normalizeConfigFile,
...
describe("resolveOutputPath", () => {
	...
	it("returns outputValue as-is when it is /dev/null or nul", async () => {
		...
		const projectMetadata = await getProjectMetadata(input);
		expect(
			await resolveOutputPath("/dev/null", input, projectMetadata, true, "md"),
		).toBe("/dev/null");
		expect(
			await resolveOutputPath("nul", input, projectMetadata, true, "md"),
		).toBe("nul");
	});
	...
	it("returns outputValue as-is when it is - (stdout)", async () => {
		...
		const projectMetadata = await getProjectMetadata(input);
		expect(
			await resolveOutputPath("-", input, projectMetadata, true, "md"),
		).toBe("-");
	});
...
describe("resolveFileConfigAtInput", () => {
	it("loads fln.json when present", async () => {
		const input = await mkdtemp(join(tmpdir(), "fln-config-primary-"));
		await writeFile(
			join(input, "fln.json"),
			JSON.stringify({ output: "primary.md" }, null, "\t"),
		);

		const result = await resolveFileConfigAtInput(input);
		expect(result.loaded).toBe(true);
		expect(result.configPath).toBe(join(input, "fln.json"));
		expect(result.fileConfig.output).toBe("primary.md");
	});

	it("does not load .fln.json when fln.json is absent", async () => {
		const input = await mkdtemp(join(tmpdir(), "fln-config-dot-ignored-"));
		await writeFile(
			join(input, ".fln.json"),
			JSON.stringify({ output: "legacy.md" }, null, "\t"),
		);
...
describe("loadConfigFile", () => {
	it("rejects unknown keys", async () => {
		const input = await mkdtemp(join(tmpdir(), "fln-config-unknown-"));
		const configPath = join(input, "fln.json");
		await writeFile(configPath, JSON.stringify({ excludePatterns: ["*.txt"] }));

		const result = await loadConfigFile(configPath);
		expect(result.loaded).toBe(false);
		expect(result.parseError).toBe(
			`Unknown config key "excludePatterns" in ${configPath}.`,
		);
	});

	it("rejects legacy noTree key", async () => {
		const input = await mkdtemp(join(tmpdir(), "fln-config-notree-"));
		const configPath = join(input, "fln.json");
		await writeFile(configPath, JSON.stringify({ noTree: true }));

		const result = await loadConfigFile(configPath);
		expect(result.loaded).toBe(false);
		expect(result.parseError).toBe(
			`Unknown config key "noTree" in ${configPath}.`,
		);
	});

	it("rejects legacy includeTree key", async () => {
		const input = await mkdtemp(join(tmpdir(), "fln-config-includetree-"));
		const configPath = join(input, "fln.json");
		await writeFile(configPath, JSON.stringify({ includeTree: false }));

		const result = await loadConfigFile(configPath);
		expect(result.loaded).toBe(false);
		expect(result.parseError).toBe(
			`Unknown config key "includeTree" in ${configPath}.`,
		);
	});
});

describe("normalizeConfigFile", () => {
	it("throws FlnError INVALID_CONFIG for invalid maxTokens string", () => {
		expect(() => normalizeConfigFile({ maxTokens: "nope" })).toThrow(FlnError);
		expect(() => normalizeConfigFile({ maxTokens: "nope" })).toThrow(
			/Invalid maxTokens/,
		);
		try {
			normalizeConfigFile({ maxTokens: "nope" });
			expect.unreachable();
		} catch (error) {
