...
describe("toCanonicalRelative", () => {
	...
	it("handles Windows-style paths when base is Windows", () => {
		...
		expect(toCanonicalRelative(posix.join("src", "nested", "file.ts"), windowsBase)).toBe("src/nested/file.ts");
	});
...
describe("toPosixPath", () => {
	...
	it("handles mixed separators", () => {
		expect(toPosixPath(String.raw`src\file/mixed.ts`)).toBe("src/file/mixed.ts");
	});
});
...
describe("resolveFromBase", () => {
	...
	it("resolves relative path against base", () => {
		expect(resolveFromBase("dist/output.json", base)).toBe(join(base, "dist", "output.json"));
	});
	
	it("handles ./ prefix", () => {
		expect(resolveFromBase("./output.json", base)).toBe(join(base, "output.json"));
	});
...
describe("getNullishOutput", () => {
	it("returns platform-specific null device", () => {
		const result = getNullishOutput();
		expect(result === "/dev/null" || result === "nul").toBe(true);
	});
});
...
describe("isStdoutOutput", () => {
	it("returns true for -", () => {
		expect(isStdoutOutput("-")).toBe(true);
	});
	
	it("returns false for other paths", () => {
		expect(isStdoutOutput("/dev/stdout")).toBe(false);
		expect(isStdoutOutput("")).toBe(false);
	});
});
